...
class Case_model extends CI_Model {
  ...
  function select_duedocuments($cid) { // NO COLUMNS FOR DUE DATE. temporary due date is 14 days from date received  ----  MALAMANG YUNG DATE NA COLUMN YUNG DUE DATE KASI IMPORTANT DATES NGA YUN. SA DATE NA YUN MAY DEADLINE NA DOCUMENT. ISIP ISIP DIN PAG MAY TIME. OR SHOULD I SAY, FOCUS FOCUS DIN, AYUSIN PRIORITIES HINDI PURO LANDI.
    /* WTF IS THIS? MASABI LANG NA MAY GAWA? OO NGA NO, KASI "PARA LANG MASABI NA MAAYOS NA" $query = $this->db->query("SELECT `legaldocument`.*, `case`.`caseNum` FROM `legaldocument` JOIN `case_people` ON `case_people`.`caseID` = `legaldocument`.`caseID` JOIN `case` ON `case`.`caseID` = `case_people`.`caseID`WHERE `case_people`.`personID` = $pid AND dateissued > 0 AND CURDATE() <= ADDDATE(date(legaldocument.datereceived) , 14)"); */
    $query = $this->db->query("SELECT * 
      FROM  `schedule` 
      WHERE  `title` LIKE  '%Deadline for document:%'
      AND `caseID` = $cid");
    return $query->result();
  }
