            ...
            <table id="action<?= $x ?>table" class="table table-condensed" style="background-color:white;">
              ...
              <?php foreach (${'actionplan_s' . $x} as $action) : ?>
                <tr id="actionTableRow_<?= $action->actionplanID ?>">
                  <td>
                      <input name='action1[]' class='cbactionstage1 <?php if ($actionplanstatus == null) { ?> disable <?php } ?>' type='checkbox' value="<?= $action->actionplanID ?>" style='margin: 0px 5px 0px 10px;' onclick="actionclick(<?= $action->actionplanID ?>, 1, <?= $case->stage ?>)" <?php if ($action->status == 1) { ?> checked disabled <?php } ?> />
                  </td>
                  <td>
