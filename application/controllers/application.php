...
//                $data['non_specialized'] = $this->People_model->select_non_specialized(1);
    ...
    function accept($cid) {
        ...
        $logassignlawyers = array(
            'caseID' => $cid,
            'action' => "Assigned to lawyer: " . $lawyersforlog,
            'dateTime' => $datetimenow,
            'stage' => $this->Case_model->select_case($cid)->stage,
            'category' => 'PEOPLE'
        );
        $this->Case_model->insert_log($logassignlawyers);
        ...
        $logassigninterns = array(
            'caseID' => $cid,
            'action' => "Assigned to intern/s: " . $internsforlog,
            'dateTime' => $datetimenow,
            'stage' => $this->Case_model->select_case($cid)->stage,
            'category' => 'PEOPLE'
        );
        $this->Case_model->insert_log($logassigninterns);

        $data['interns'] = $this->Case_model->select_caseinterns($cid);
